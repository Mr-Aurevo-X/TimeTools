...
"""Optional GitHub updater for TimeTools.

Legal: updates are not guaranteed (no SLA). Frozen/exe mode prefers the
GitHub Release asset TimeTools.exe; source mode uses git pull (clone) or the
...
SETTINGS_NAME = f"{APP_SLUG.lower()}-settings.json"
FINISH_SCRIPT = f"_{APP_SLUG.lower()}_update_finish.cmd"
...
def _apply_via_source_zip(release: dict, remote: str) -> dict[str, Any]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix=f"{APP_SLUG.lower()}-src-"))
    try:
...
def _apply_via_exe_asset(release: dict, remote: str) -> dict[str, Any]:
    ...
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix=f"{APP_SLUG.lower()}-upd-"))
